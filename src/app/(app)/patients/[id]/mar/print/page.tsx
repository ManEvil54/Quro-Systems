        ...
        return (
          <React.Fragment key={`page-pair-${pageIdx}`}>
            ...
            <div className="print-page page-front">
              ...
              {/* Grid Table */}
              <table className="w-full border-collapse border border-black text-[10.5px] table-fixed">
                <thead>
                  <tr className="bg-sky-50 text-[9.5px] uppercase tracking-wider font-black h-8 text-slate-800">
                    ...
                    {daysArray.map((day) => (
                      <th key={day} className="border border-black w-[2%] text-center font-mono text-[8.5px]">{day}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                          ...
                          <tr key={`${record.id}-${rowIndex}`} className="h-5 break-inside-avoid">
                            {rowIndex === 0 && (
                              <td rowSpan={subRowsCount} className="border border-black p-1.5 align-top bg-white print-border overflow-hidden">
                                <div className="text-[8.5px] text-rose-600 font-bold mb-0.5">
                                  {record.start_date}
                                </div>
                                <div className="font-black uppercase text-[10px] text-black leading-tight truncate">
                                  {record.title}
                                  {record.is_psychotropic && (
                                    <span className="ml-1 px-1 bg-red-100 text-red-800 border border-red-200 rounded text-[7.5px] font-black uppercase tracking-tight">
                                      Psych
                                    </span>
                                  )}
                                </div>
                                <div className="text-[9px] text-slate-500 font-mono mt-0.5 leading-tight truncate-multi">
                                  {record.details}
                                </div>
                              </td>
                            )}
                            <td className="border border-black text-center font-mono text-[9px] bg-white print-border font-bold text-slate-800 h-5">
                              {displayTimes[rowIndex] || ""}
                            </td>
                            {daysArray.map((day) => (
                              <td key={day} className="border border-black bg-white print-border text-center align-middle font-mono text-[8px] text-slate-200 relative w-6 h-5 p-0 select-none">
                                {day}
                              </td>
                            ))}
                          </tr>
                        ...
                        <tr key={`blank-template-${idx}-${rowIndex}`} className="h-5 break-inside-avoid">
                          {rowIndex === 0 && (
                            <td rowSpan={4} className="border border-black p-1.5 align-bottom bg-white print-border overflow-hidden">
                              ...
                              <div className="text-[8px] text-slate-400 font-mono italic mt-1 leading-tight uppercase font-semibold">
                                Additional Order / Medication (Scribe Manually)
                              </div>
                            </td>
                          )}
                          <td className="border border-black text-center font-mono text-[9px] bg-white print-border h-5" />
                          {daysArray.map((day) => (
                            <td key={day} className="border border-black bg-white print-border text-center align-middle font-mono text-[8px] text-slate-200 relative w-6 h-5 p-0 select-none">
                              {day}
                            </td>
              ...
              <div className="mt-2 break-inside-avoid">
                <div className="border border-black p-2 grid grid-cols-4 gap-x-4 gap-y-1 text-[9px] uppercase font-mono break-inside-avoid bg-white">
              ...
              <div className="print-page-footer mt-2 border border-black text-[10px] uppercase font-mono tracking-tight divide-y divide-black bg-white">
                {/* Row 1 */}
                <div className="grid grid-cols-12 divide-x divide-black">
                  <div className="col-span-3 p-1 flex flex-col justify-between">
                    <span className="text-[7.5px] text-slate-400">Physician/Alt. Physician</span>
                ...
                <div className="grid grid-cols-12 divide-x divide-black">
                  ...
                  <div className="col-span-1 p-1 flex flex-col justify-between text-center">
                    <span className="text-[7.5px] text-slate-400">Sex</span>
                    <span className="font-black text-slate-800">{patientGender}</span>
                  </div>
                  <div className="col-span-1 p-1 flex flex-col justify-between">
                    <span className="text-[7.5px] text-slate-400">DOB</span>
                    <span className="font-black text-slate-800">{patientDobFormatted}</span>
                  </div>
                  <div className="col-span-3 p-1 flex flex-col justify-between">
                    <span className="text-[7.5px] text-slate-400">Allergies/Notes</span>
            ...
            <div className="print-page page-back">
              ...
              <div className="mb-2">
                <div className="bg-[#0284c7] text-white p-1 text-center text-[11px] font-black uppercase tracking-widest mb-1 rounded">
                  Daily Vital Signs Record • Back Copy
                </div>
                <table className="w-full border-collapse border border-black text-[9.5px] uppercase font-mono table-fixed">
                  <thead>
                    <tr className="bg-slate-100 text-[8.5px] h-6 text-slate-700">
                  ...
                  <tbody>
                    ...
                    <tr className="h-10">
                      <td className="border border-black p-1 font-black bg-slate-50 text-[9px] leading-tight">
                        Blood Pressure
                        <span className="block text-[6.5px] text-slate-400 lowercase font-normal leading-none mt-0.5">
                          systolic / diastolic
                        </span>
                      </td>
                      {daysArray.map((day) => (
                        <td key={day} className="border border-black bg-white print-border relative w-6 h-10 p-0">
                          ...
                          <span className="absolute top-0.5 right-1 text-[6.5px] text-slate-400 leading-none">S</span>
                          <span className="absolute bottom-0.5 left-1 text-[6.5px] text-slate-400 leading-none">D</span>
                        </td>
                      ))}
                    </tr>
                    {/* Pulse Row */}
                    <tr className="h-5">
                      <td className="border border-black p-1 font-black bg-slate-50 text-[9px]">Pulse</td>
                      {daysArray.map((day) => (
                    ...
                    {/* Weight Row */}
                    <tr className="h-5">
                      <td className="border border-black p-1 font-black bg-slate-50 text-[9px]">Weight (lbs)</td>
                      {daysArray.map((day) => (
                        <td key={day} className="border border-black bg-white print-border" />
                      ))}
                    </tr>
                    {/* Temperature Row */}
                    <tr className="h-5">
                      <td className="border border-black p-1 font-black bg-slate-50 text-[9px]">Temperature</td>
                      {daysArray.map((day) => (
                        <td key={day} className="border border-black bg-white print-border" />
                      ))}
                    </tr>
              ...
              <div>
                ...
                <table className="w-full border-collapse border border-black text-[9.5px] uppercase font-mono table-fixed">
                  <thead>
                    <tr className="bg-slate-100 text-[8.5px] font-bold text-center h-6 text-slate-700">
